...
public class TableAllTasks {
	...
	public DataGrid<Task_Data> InitTable() {
		...
		// cl description
		Column<Task_Data, String> clDescription = new Column<Task_Data, String>(
				new ClickableTextCell()) {
			@Override
			public String getValue(Task_Data object) {

				if (object != null && object.getDescription() != null) {
					String stReturn = object.getDescription();
					if (stReturn.length() >= 100) {
						return stReturn.substring(0, 100) + "....";
					} else {
						return stReturn;
		...
		gridTasks.addColumn(clDescription, appConstants.ViewDescription());

		// cl Sender
		Column<Task_Data, String> clSender = new Column<Task_Data, String>(
				new ClickableTextCell()) {
			@Override
			public String getValue(Task_Data object) {
		...
		gridTasks.addColumn(clSender, appConstants.ViewTaskSender());

		// cl Sender
		Column<Task_Data, String> clRecipient = new Column<Task_Data, String>(
				new ClickableTextCell()) {
			@Override
			public String getValue(Task_Data object) {
		...
		gridTasks.addColumn(clRecipient, appConstants.ViewTaskRecipient());
